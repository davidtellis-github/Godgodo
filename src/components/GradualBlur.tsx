...
function debounce<T extends (...args: unknown[]) => void>(fn: T, wait: number): T {
  ...
  return ((...a: unknown[]) => { clearTimeout(t); t = setTimeout(() => fn(...a), wait); }) as T;
}
...
function useResponsiveDimension(responsive: boolean, config: Config, key: 'height' | 'width') {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (w <= 480 && config[`mobile${key[0].toUpperCase() + key.slice(1)}`]) v = config[`mobile${key[0].toUpperCase() + key.slice(1)}`] as string;
      else if (w <= 768 && config[`tablet${key[0].toUpperCase() + key.slice(1)}`]) v = config[`tablet${key[0].toUpperCase() + key.slice(1)}`] as string;
      else if (w <= 1024 && config[`desktop${key[0].toUpperCase() + key.slice(1)}`]) v = config[`desktop${key[0].toUpperCase() + key.slice(1)}`] as string;
      setValue(v);
    };
...
function GradualBlur(props: GradualBlurProps) {
  ...
  const config = useMemo<Config>(() => {
    ...
    return mergeConfigs(DEFAULT_CONFIG, presetConfig, props as Partial<Config>);
  }, [props]);
...
if (typeof document !== 'undefined') {
  ...
  if (!document.getElementById(id)) {
    ...
    s.id = id;
    s.textContent = `.gradual-blur{pointer-events:none;transition:opacity 0.3s ease-out}.gradual-blur-parent{overflow:hidden}.gradual-blur-inner{pointer-events:none}`;
